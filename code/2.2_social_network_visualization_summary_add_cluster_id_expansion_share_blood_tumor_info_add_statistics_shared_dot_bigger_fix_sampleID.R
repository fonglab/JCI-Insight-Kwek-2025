...
# for (pat_id in c("a5")) {
  ...
  dim(input_raw_data)
  dir.create("distance_matrix_by_patient", showWarnings = FALSE)
...
sink("./Bladder_TCR_match_paper_cell_level_w_all_cluster_stats/soc atezo patient level summary table info_a4_shared_dot_bigger.txt")
# pat_id_list <- c( 'a4')
...
for (pat_id in pat_id_list) {
  for (comp_id in comp_list) {
    for (tissue_id in tissue_list) {
      ...
      # select only shared cells
      input_raw_data_select_post_tumor <- input_raw_data_select[input_raw_data_select$tissue %in% c( tissue_id ) & input_raw_data_select$comp %in% c(comp_id) & input_raw_data_select$blood_tumor %in% c("shared"),]
      ...
      dir.create("social_network_plot_on_each_patient_cell_level", showWarnings = FALSE)
      pdf(paste0("./social_network_plot_on_each_patient_cell_level/social network plot on each patient cell level, subset data on ", comp_id, " patient ID ",pat_id, " tissue ", tissue_id, "_shared_dot_bigger.pdf"))
      ### best pick layout
      set.seed(9999)
      ...
      print(ggraph(net,layout = l)+
              ...
              geom_node_point(aes(color = expansion),size=2) + 
      ...
      print(ggraph(net,layout = l)+
              geom_edge_link0(width=0.1,colour="grey",alpha =0.8 )+
              geom_node_point(aes(color = treatment),size=2) +
              theme_graph(base_family="sans") +
              ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      ...
      print(ggraph(net,layout = l)+
              geom_edge_link0(width=0.1,colour="grey",alpha =0.8 )+
              geom_node_point(aes(color = blood_tumor),size=2) + 
              # geom_node_point(aes(color = cell_type,size = cloneCount)) + scale_size(range = c(0.1,log(max(cloneCount))/2.5)) +
              theme_graph(base_family="sans") +
              ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      ...
      # # list all cell type on the label
      # #Create a custom color scale
      # library(RColorBrewer)
      # myColors <- brewer.pal(12,"Set3")
      # names(myColors) <- levels(sub_input_raw_data$cell_type)
      # cell_type_color = myColors[as.character(sub_input_raw_data$cell_type)]
      # 
      # colScale <- scale_colour_manual(name = cell_type,values = myColors)
      # ## the custom function using Color Brewer
      # cols_f <- colorRampPalette(RColorBrewer::brewer.pal(12,"Set3"))
      
      # cell_type <- levels(sub_input_raw_data$cell_type)
      print(ggraph(net,layout = l)+
              geom_edge_link0(width=0.1,colour="grey",alpha =0.8 )+
              # geom_node_point(aes(color = cols_f(length(cell_type))),size=2) +
              geom_node_point(size=2) +
              # scale_fill_manual(values =names(myColors))+
              theme_graph(base_family="sans") +
              ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      ...
      print("the composition of the largest clusters.")
      ...
      # print(ggraph(net,layout = l)+
      #         geom_edge_link0(width=0.1,colour="grey",alpha =0.8 )+
      #         geom_node_point(aes(color = GZMB),size=2) +
      #         theme_graph(base_family="sans") +
      #         ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      ...
      # print(ggraph(net,layout = l)+
      #         geom_edge_link0(width=0.1,colour="grey",alpha =0.8 )+
      #         geom_node_point(aes(color = GZMK),size=2) +
      #         theme_graph(base_family="sans") +
      #         ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      ...
      # print(ggraph(net,layout = l)+
      #         geom_edge_link0(width=0.1,colour="grey",alpha =0.8 )+
      #         geom_node_point(aes(color = GZM),size=2) +
      #         theme_graph(base_family="sans") +
      #         ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      ...
      print(ggraph(net,layout = l)+
              ...
              geom_node_point(aes(color = tissue),size=2) +
              theme_graph(base_family="sans") +
              ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      ...
      # print(ggraph(net,layout = l)+
      #         geom_edge_link0(width=0.1,colour="grey",alpha =0.8 )+
      #         geom_node_point(aes(color = Tregs),size=2) +
      #         theme_graph(base_family="sans") +
      #         ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      ...
      print(ggraph(net,layout = l)+
              geom_edge_link0(width=0.1,colour="grey",alpha =0.8 )+
              geom_node_point(aes(color = comp),size=2) +
              theme_graph(base_family="sans") +
              ggtitle(paste0('Alpha and Beta chain, Patient: ', pat_id, " ", comp_id," ", tissue_id)) )
      dev.off()
      ...
      # 
      # write.csv(sub_input_raw_data, file = paste0( "./Bladder_TCR_match_paper_cell_level_w_all_cluster_stats/Bladder_TCR_match_paper_cell_level_w_all_cluster_stats_","subset_data_on_", comp_id, "_patient_ID_",pat_id, "_tissue_", tissue_id, "_shared.csv"), row.names=FALSE)
      # 
      # 
      # 
      # 
      ...
      #   write.csv(membership_table, file = paste0("./cluster_level_info/cluster_level_info_match_paper_","subset_data_on_", comp_id, "_patient_ID_",pat_id, "_tissue_", tissue_id, "_shared.csv"), row.names=FALSE)
